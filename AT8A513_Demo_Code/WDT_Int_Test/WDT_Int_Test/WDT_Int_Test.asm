...
; Time: 16:45
; 说明：PB0:Timer0中断输出。
;		PB2:若 WDT 选用INT，则为WDT中断转态输出。
;		PB2:若 WDT 选用Reset ，高电平为工作模式，低电平为复位时间。
; =========================================================================
...
Main_Prog:
		...
		MOVIA		0x41			;; ENABLE WDT/T0 Interrupt
		MOVAR		INTE
...
; Madify Data: 2014.12.11
		MOVIA		0x00			;; 设定为 WDT(B7)=1:EN/0:DIS, PB0(B6)=1:INT/0:GPIO, LVR(B3)=1:EN/0:DIS
		MOVAR		PCON
		
		MOVIA		0xBF
		MOVAR		INTF			;; Clr WDT Flag
		MOVIA		0x09			
 		T0MD						;; 设定WDT 中断分频(1:4)，中断时间为 15m*4=60ms。"注意ConfigWord：Word Event 要选择为 Interrupt"
 									;; 设定WDT 复位分频(1:2)，中断时间为 15m*2=30ms。"注意ConfigWord：Word Event 要选择为 Reset"
 		MOVIA		0x80			;; 设定为 WDT(B7)=1:EN/0:DIS, PB0(B6)=1:INT/0:GPIO, LVR(B3)=1:EN/0:DIS
		MOVAR		PCON
		RET
